...
#'
#' @param object A data.table with class "matching_study_cohort" (output from `build_study_cohort()`).
...
summary.matching_study_cohort <- function(object, ...) {
  ...
  invisible(
    list(
      ...
      control_usage_distribution = if (nrow(controls) > 0L) control_usage_dist else NULL
    )
  )
}
...
#'
#' By default, this produces a stacked histogram of matched episodes by
#' `group` over `T0` (index date), with automatic binning. You can also provide
#' any other variable in `by_var` to generate a stacked bar plot over that
#' variable's levels.
#'
#' @param x A data.table with class "matching_study_cohort" (output from `build_study_cohort()`).
...
plot.matching_study_cohort <- function(x, y, by_var = "T0", ...) {
  ...
  # For T0 default plotting, prefer Date axis and fail explicitly if conversion is impossible.
...
.cb_sensible_hist_bins <- function(values) {
  ...
  # Keep binning readable and stable across typical cohort sizes.
